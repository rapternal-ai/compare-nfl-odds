...
import { executePaperExit, expirePaperOrders, fillActivePaperOrder, getActivePaperOrders, getPaperControlState, revalidateAndExecutePaperEntry } from "@/db/repository";
...
export async function executePaperScan(sql: Database, records: ScanRecord[], marketData: MarketDataProvider, now = new Date()) {
  ...
  for (const record of records) await executePaperExit(sql, record, risk, now);
